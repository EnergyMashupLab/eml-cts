...
public class LmeSocketServer extends Thread {
	 ...
	 */
	public static ConcurrentHashMap<Long, EiCreateTransactionPayload> eiCreateTransactionMatchNumberMap = new ConcurrentHashMap<Long, EiCreateTransactionPayload>();
	...
	final MarketCreateTransactionPayloadDecoder marketCreateTransactionPayloadDecoder = new MarketCreateTransactionPayloadDecoder();
...
// 				" '" + Thread.currentThread().getName() + "'");
		 ...
		 * 
		 * If the matchNumber is in the Map (the first of two transactions has been
		 * processed) we send the one stored in the Map (and delete the entry).
		 * 
		...
		while (true) {
			...
			try {
				...
				if (eiCreateTender == null) {
					 ...
					 * 
					 * When [Cts]TenderId has a value in the TenderId map we extract the original
					 * tender, and build the EiCreateTransactionPayload using original parties and
					 * TenderId, with cleared quantity and price.
					 */
					...
					eiCreateTransaction = new EiCreateTransactionPayload(transaction, eiCreateTender.getPartyId(),
							eiCreateTender.getCounterPartyId());

					logger.trace("LmeSocketServer EiCreateTransaction " + eiCreateTransaction.toString());
					/*
					 * Determine whether a previous EiCreateTransaction built from the same Parity
					 * matchNumber was saved in eiCreateTransactionMatchNumberMap
					 * 
					 * If so, send it then send the current EiCreateTransactionPay;oad If not, save
					...
					if (matchEiCreateTransaction == null) {
						...
						matchEiCreateTransaction.setCounterPartyId(eiCreateTransaction.getPartyId());
						logger.trace("matchEiCreateTransaction after update " + matchEiCreateTransaction.toString());

						eiCreateTransaction.setCounterPartyId(matchEiCreateTransaction.getPartyId());
						...
						logger.debug("LmeSocketServer enqueued " + " " + matchEiCreateTransaction.toString());
