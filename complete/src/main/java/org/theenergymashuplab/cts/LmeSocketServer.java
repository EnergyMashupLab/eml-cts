...
public class LmeSocketServer extends Thread {
	...
	/*
	 * map is from (Long)MarketCreateTransactionPayload.getMatchNumber() to the EiCreateTenderPayload built from the
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				payload = mapper.readValue(jsonReceived, MarketCreateTransactionPayload.class);
				...
				if (eiCreateTender == null) {
					 ...
					 * 
					 * When [Cts]TenderId has a value in the TenderId map we extract the original tender, and build the
					 * EiCreateTransactionPayload using original parties and TenderId, with cleared quantity and price.
					 */
					...
					eiCreateTransaction = new EiCreateTransactionPayload(transaction, eiCreateTender.getPartyId(),
							eiCreateTender.getCounterPartyId(), new TransactionIdType());
