...
public class LmeSendTransactions extends Thread {
	...
	private static final Logger logger = LogManager.getLogger(LmeSendTransactions.class);
	final RestTemplateBuilder builder = new RestTemplateBuilder();

	/*
	 * Take first EiCreateTransactionPayload from eiCreateTransactionQueue and Post it to the LMA.
	 */
	@Override
	public void run() {
		...
		while (true) {
			...
			if (eiCreateTransaction != null) {
				...
				logger.info("eiCreateTransaction null");
	...
	public EiCreatedTransactionPayload postClientCreateTransaction(
		...
		logger.debug("LmeSendTransactions.postClientCreateTransaction " + tempTransaction.toString());
		/*
		...
		tempPostResponse = restTemplate.postForObject("http://localhost:8080/lma/createTransaction",
				eiCreateTransaction, EiCreatedTransactionPayload.class);
