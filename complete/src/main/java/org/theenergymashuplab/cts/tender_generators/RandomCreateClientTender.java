...
public class RandomCreateClientTender {
	...
	RandomCreateClientTender() {
		...
		for (int i = 0; i < 24; i++) {
			...
			// " name from BrInstant " + instrumentNames[i] +
					" name from BrInterval " + instrumentNamesFromInterval[i] + " " + instants[i].toString());
		}
	...
	public ClientCreateTenderPayload randomTender() {
		...
		randTender = new ClientCreateTenderPayload(side, randQuantity, price, startTime, 60 * 60);
